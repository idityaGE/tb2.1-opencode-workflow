...
- Treat `ruff check --select I,UP,PLW,ISC,E,Q,C,RUF100` as style guidance for Python verifier/helper code. Enforce only the platform-mandatory Ruff command in validation; this broader selector output is advisory and must not drive rewrites that change task semantics, NOP failure behavior, or oracle success behavior.
