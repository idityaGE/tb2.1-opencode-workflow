...
- Before drafting, follow the centralized category policy in `tb2-hard-task-author`. Make the first sentence, objective, and expected deliverable naturally match the selected primary activity without announcing a taxonomy label.
- If that policy classifies the honest prompt as currently blocked, stop and redesign it. Do not use unnatural wording or metadata changes to make a blocked task look allowed.
