...
- Locate the matching task from the delegated folder name and feedback. If a truncated folder prefix is not unique, return the candidates to the parent as `WAITING` rather than asking from the subagent. Run `.opencode/scripts/tb2_task_state.sh --task tasks/<task_name> --write-cache` before and after edits.
...
- The update helper retries a failed `stb submissions update` at most five times in one mode. If upload prep changes files, validate again before invoking it. Stop after helper success or final failure.
