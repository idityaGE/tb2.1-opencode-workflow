...
- `repair-and-check` when any of these is present: `TRIVIAL` or `EASY`; an explicit failed-difficulty message; a non-solvable status; oracle failure; any failed quality check other than `Task Instruction Sufficiency` or `not_applicable`; or a new actionable reviewer note. Repair all supported issues, validate, and upload with `--no-send-to-reviewer`.
