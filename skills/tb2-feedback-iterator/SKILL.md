...
The private JSON must include: `prior_difficulty` (string), `prior_agent_performance` (object), `successful_trace_sources` (string array), `common_success_strategy` (string), `prior_failed_hardening` (string array, `[]` when none), `starting_state_files_changed` (string array), `defect_families` (object array), `contract_changes` (string array), and `removed_or_replaced_shallow_complexity` (string array). Each defect family records string fields `name`, `root_cause`, `strategy_invalidated`, `oracle_signal`, and `verifier_signal`, plus string arrays `source_delta` and `interacts_with`. Never use prose strings where an array is required.
...
- Broad Ruff selectors remain advisory and separate from mandatory Ruff status. Their full output is saved under `.tb2-cache/tb2-validation/advisory-ruff/`; validation prints only compact rule counts and the report path. Do not open the full report unless a concrete task repair requires it. Local validation may claim structural validity, contract alignment, mandatory Ruff status, NOP result, and oracle result only. Say: `Local validation passed; platform difficulty and quality reevaluation pending.` Never claim local difficulty, empirical frontier-strategy invalidation, platform quality, or that the task is fixed.
