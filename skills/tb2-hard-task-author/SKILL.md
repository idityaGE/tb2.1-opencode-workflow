...
- Do not create task-level or environment docs/help of any kind: no `README*`, `docs/`, `doc/`, `documentation/`, guides, manuals, notes, walkthroughs, local specs, or reference excerpts. Required behavior belongs in a concise, minimal `instruction.md` only.
...
- Do not use local specs or helper docs. If APIs, protocols, schemas, or business rules are needed for fairness, state only the minimal observable contract in `instruction.md`.
...
- Avoid README/spec over-explaining entirely; concise prompts plus realistic code are required.
...
Red flags:
- Vague prompts, hidden requirements only in tests, arbitrary gotchas, misleading comments, one obviously broken function, prompt text that points to a file or bug, fixture/test names that reveal the edge case, README/docs/spec walkthroughs, helper notes, giant instruction dumps, dead-file noise, runtime internet dependence, or static expected outputs that can be hardcoded.

First-go hardness gates:
- Before authoring, make a private 5-8 line difficulty blueprint: bug layers, why agents miss them, evidence trail, verifier coverage, oracle repair path, and duplicate-risk note.
...
- Before full validation, self-check that NOP should fail, oracle should pass, tests map to prompt, prompt has no hints, Docker is compliant, duplicate scan is clean, and solution derives the repair.
