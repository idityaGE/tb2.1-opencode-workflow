...
- Target difficulty: `medium` or `hard`, selected honestly from the documented pass-rate thresholds. Prefer `hard` when viable and research enough domain/task evidence to justify that rating, but do not inflate medium ideas. Do not create easy, trivial, or unknown-difficulty tasks.
...
- Before full validation, complete the private four-way contract alignment audit from `tb2-tests` with zero oracle omissions, zero uncovered requirements, zero ungrounded verifier behaviors, and meaningful NOP failure. Also self-check that every hidden layer is verifier-relevant, the prompt and approved contracts have no hints, Docker is compliant, the duplicate scan is clean, and the solution derives the complete repair.
