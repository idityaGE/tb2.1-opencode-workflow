...
- Apply this gate whenever platform evidence rates an existing task `EASY` or `TRIVIAL`. Read the available successful and near-success frontier-agent summaries or traces before redesigning. Privately record the common successful approach, which behavior it already handled, and why the current agent-visible starting state made that approach reliable.
- Reapply the full difficulty blueprint to the revised task. Count independent root-cause failure families, not requirements, test functions, fixtures, or variants of one edge case. A `TRIVIAL` result requires a structural redesign of the starting-state failure topology; an `EASY` result requires invalidating the observed common success path and adding or deepening an orthogonal non-local failure family.
- Materially change the agent-visible implementation or runtime starting state under `environment/`. Changes limited to `instruction.md`, `tests/`, `solution/`, or metadata are not a hardness repair and must block the checks upload. Prefer a mostly working system with subtle interacting defects over an empty stub, wholesale missing implementation, or a prompt that invites a clean rewrite from a complete specification.
- Before validation, produce private hardening evidence that maps each revised hidden defect family to its agent-visible source delta, interaction with another layer, observed successful strategy it defeats, oracle repair, and independent verifier signal. Block the revision if the evidence only adds contract volume, more cases for one root cause, decorative complexity, or behavior the starter already handled incorrectly in an obvious way.
