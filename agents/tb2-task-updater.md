...
14. After edits, complete the private bidirectional audit from `tb2-tests` plus the instruction sufficiency, ambiguity-ledger, and complete/domain-blind category-surface reviews from `tb2-instruction` for `instruction.md`, every public contract, and the verifier. Require zero uncovered requirements, zero ungrounded tested behaviors, every critical edge class covered, and every intended hidden layer verifier-relevant. If an instruction change creates an uncovered obligation, update the verifier and use full validation; `fast-only` is allowed only when unchanged tests already prove the complete revised contract.
