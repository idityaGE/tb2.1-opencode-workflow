...
9. Keep all required behavior specified without turning `instruction.md` into a specification. If feedback says the instruction is underspecified, clarify the prompt's objective, expected outcome, scope, absolute paths, or authoritative-document references. Put necessary detailed declarative behavior in an approved environment document, without revealing solution steps, exact bug locations, edge-case names, or implementation hints.
...
13. Use targeted web research only when it helps resolve a concrete feedback issue around external specs, unusual languages, recent tool behavior, or library documentation. Use that research during revision only; do not vendor source material. Under `tb2-hard-task-author`, add `environment/README.md` for necessary realistic system context/interfaces and at most one `environment/spec.md` or `environment/rule.md` for necessary normative behavior; concrete reviewer feedback may explicitly request one. Generic sufficiency or coverage feedback is not an explicit request. Preserve reviewer evidence in the progress summary, never put it in the task, reference each approved document by absolute path from the prompt, and keep all documents minimal, realistic, declarative, and free of repair guidance, known issues, verifier-shaped edge-case lists, or seeded bug clues. Keep `allow_internet = false` unless platform docs explicitly require otherwise.
