...
3. For every returned row, invoke `tb2-task-updater` with the submission ID, displayed folder name, and all user constraints. The updater owns feedback classification, edits, validation, upload mode, retries, rubric handoff, and its per-submission result.
