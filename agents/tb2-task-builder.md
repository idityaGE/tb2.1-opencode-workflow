...
10. Run the first-go quality gate before full validation. Complete the private four-way contract alignment audit from `tb2-tests`, the clause-by-clause oracle review from `tb2-solution`, and the instruction sufficiency, ambiguity-ledger, and complete/domain-blind category-surface reviews from `tb2-instruction`. Require zero oracle omissions, zero uncovered requirements, zero ungrounded verifier behaviors, no vacuous or source-inspection tests, coverage of every critical edge class, and an independent behavioral rejection path plus integrated coverage for every hidden layer. Confirm NOP should fail for missing required behavior rather than infrastructure failure, oracle should satisfy every instruction/README/spec/rule clause through a real derived implementation, no-op/hardcoded/partial/visible-fixture-only solutions are rejected, and verifier assertions establish semantic correctness rather than only format or shape. Also confirm tests are deterministic and anti-cheating boundaries hold; `tests/test.sh` follows the exact `tb2-tests` runner shape; every public contract is referenced and agent-visible; approved documents are realistic/minimal and reveal no hidden failure layer; the selected category follows `.opencode/docs/tb2/task-taxonomy.md`; Docker follows component-skill policy; the duplicate scan is clean; no single local patch can satisfy the contract; and the solution derives the repair rather than hardcoding results. If documentation makes the task shallow, deepen implementation interactions rather than deleting fair requirements. Then run `.opencode/scripts/tb2_preflight_task.sh --task tasks/<task_name>` to catch fast structural/ruff failures before expensive NOP/oracle. Fix obvious misses before running full validation.
...
- Treat every normative clause used from an approved environment contract as part of the oracle and verifier contract. Audit it through the private four-way contract alignment matrix, but never expose requirement IDs or defect mappings in task files.
