...
Required workflow:
1. Load and follow these skills as the compact source of TB2 policy: `tb2-hard-task-author`, `tb2-task-toml`, `tb2-dockerfile`, `tb2-instruction`, `tb2-solution`, `tb2-tests`, and `tb2-field-answers`.
2. Do not bulk-read `.opencode/docs/tb2` during normal builds. Read targeted docs only when a selected task needs a special case not covered by the skills, such as UI, milestones, rubric edge cases, or a Docker/policy exception.
...
5. Before writing files, draft a private 5-8 line difficulty blueprint covering: bug layers, why agents will miss them, evidence that reveals them, verifier coverage, oracle repair path, and why the task is not a near-duplicate. Do not put this blueprint in task files or final output.
...
7. Initialize with `.opencode/scripts/tb2_create_task.sh --task <task_name> --template <default|ui|milestone>` unless the task already exists and the parent explicitly asked to continue it.
8. Author the task implementation, Docker environment, `instruction.md`, `task.toml`, `solution/solve.sh`, and tests. Do not create task-level or environment docs/help of any kind: no `README*`, `docs/`, `doc/`, `documentation/`, guides, manuals, notes, walkthroughs, local specs, or solver-facing reference material. The task contract must live only in a concise, minimal `instruction.md` plus realistic code behavior. The task must include at least 3 interacting hidden defects or layered failure modes, at least one non-local bug, and at least one edge case that is not exposed by the happy path. These bugs must be discoverable only through careful debugging and edge-case reasoning, with no comments, fixture names, test names, docs, or prompt text that point to the bug locations. If web research is needed for an obscure language, recent tool behavior, or an authoritative spec, use it during authoring only; never vendor that material into the task and never make solving depend on live internet.
...
10. Run the first-go quality gate before full validation: confirm NOP should fail, oracle should pass, tests map to prompt requirements, `tests/test.sh` exactly follows the `tb2-tests` runner shape and reward-tail style, prompt gives no hints, Docker follows component-skill policy, duplicate scan was focused and clean, the solution derives the repair rather than hardcoding results, and the visible task would not be classified as blocked `software-engineering`, `debugging`, or `data-processing`. Then run `.opencode/scripts/tb2_preflight_task.sh --task tasks/<task_name>` to catch fast structural/ruff failures before expensive NOP/oracle. Fix obvious misses before running full validation.
...
Hard rules:
- Do not create Python implementation tasks. Python is only for pytest verifiers. Any other implementation language is allowed when Docker and tests are deterministic.
...
- Keep instructions sufficient but make the implementation/debugging path deep: specify behavior and invariants, not repair steps, and use the `tb2-hard-task-author` sufficiency/difficulty guidance instead of adding hints when clarity is needed.
- If validation reports `[category_classifier]` predicting blocked `software-engineering`, `debugging`, or `data-processing`, rework the task's primary activity and visible wording so it genuinely fits an allowed category. Do not fix this by changing only `task.toml`, do not disguise a blocked task with misleading wording, and do not add submission IDs to any exemption list.
...
- `allow_internet = false` unless the docs explicitly require otherwise for the selected task type.
