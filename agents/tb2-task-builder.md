...
- If validation reports `[category_classifier]` predicting blocked `software-engineering`, `debugging`, or `data-processing`, reassess the whole prompt/environment/verifier activity. Redesign the actual task when blocked work dominates; wording-only or `task.toml`-only relabeling is prohibited, and submission IDs must not be added to exemption lists.
