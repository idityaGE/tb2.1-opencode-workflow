...
10. Run the first-go quality gate before full validation. Complete the private bidirectional audit from `tb2-tests` and the instruction sufficiency, ambiguity-ledger, and complete/domain-blind category-surface reviews from `tb2-instruction`; require zero uncovered requirements, zero ungrounded tested behaviors, coverage of every critical edge class, and an independent behavioral rejection path plus integrated coverage for every hidden layer. Confirm NOP should fail, oracle should pass, no-op/hardcoded/partial/visible-fixture-only solutions are rejected, `tests/test.sh` exactly follows the `tb2-tests` runner shape and reward-tail style, `instruction.md` is prompt-only but clear, every public contract is referenced and agent-visible, approved documents are realistic/minimal and reveal no hidden failure layer, and the selected category follows `.opencode/docs/tb2/task-taxonomy.md`. Also confirm Docker follows component-skill policy, the duplicate scan was focused and clean, no single local patch can satisfy the contract, and the solution derives the repair rather than hardcoding results. If documentation makes the task shallow, deepen implementation interactions rather than deleting fair requirements. Then run `.opencode/scripts/tb2_preflight_task.sh --task tasks/<task_name>` to catch fast structural/ruff failures before expensive NOP/oracle. Fix obvious misses before running full validation.
