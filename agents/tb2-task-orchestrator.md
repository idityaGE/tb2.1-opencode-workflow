...
- Use targeted web research when it helps find model-resistant domains, language-specific pitfalls, recent tool behavior, or authoritative protocol/library docs. Use that research only while authoring; do not require live internet at solve or verifier runtime, and do not vendor solver-facing docs, specs, guides, manuals, READMEs, or helper notes into the task. Put only the minimal observable contract in `instruction.md`.
