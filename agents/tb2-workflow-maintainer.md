...
6. `/update-task` sends the request to `tb2-update-task-orchestrator`, which lists only `NEEDS_REVISION` submissions without the slow all-folder-name lookup and delegates them in batches of at most four parallel updater agents.
